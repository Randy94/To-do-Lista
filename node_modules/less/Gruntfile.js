   ...
   // Sauce Labs browser
    ...
    grunt.initConfig({
        ...
        shell: {
            ...
            opts: { // test running with all current options (using `opts` since `options` means something already)
                command: [ // @TODO: make this more thorough
                    // CURRENT OPTIONS
                    // --math
                    'node bin/lessc --math=always test/less/lazy-eval.less tmp/lazy-eval.public',
                    'node bin/lessc --math=parens-division test/less/lazy-eval.less tmp/lazy-eval.public',
                    'node bin/lessc --math=parens test/less/lazy-eval.less tmp/lazy-eval.public',
                    'node bin/lessc --math=strict test/less/lazy-eval.less tmp/lazy-eval.public',
                    'node bin/lessc --math=strict-legacy test/less/lazy-eval.less tmp/lazy-eval.public',

                    // DEPRECATED OPTIONS
                    // --strict-math
                    'node bin/lessc --strict-math=on test/less/lazy-eval.less tmp/lazy-eval.public',
                ].join(' && ')
            },
            plugin: {
                command: [
                    'node bin/lessc --clean-public="--s1 --advanced" test/less/lazy-eval.less tmp/lazy-eval.public',
                    'cd lib',
                    'node ../bin/lessc --clean-public="--s1 --advanced" ../test/less/lazy-eval.less ../tmp/lazy-eval.public',
                    'cd ..',
                    // Test multiple plugins
                    'node bin/lessc --plugin=clean-public="--s1 --advanced" --plugin=autoprefix="ie 11,Edge >= 13,Chrome >= 47,Firefox >= 45,iOS >= 9.2,Safari >= 9" test/less/lazy-eval.less tmp/lazy-eval.public'
                ].join(' && ')
            },
            'sourcemap-test': { // quoted value doesn't seem to get picked up by time-grunt, or isn't output, at least; maybe just "sourcemap" is fine?
                command: [
                    'node bin/lessc --source-map=test/sourcemaps/maps/import-map.map test/less/import.less test/sourcemaps/import.public',
                    'node bin/lessc --source-map test/less/sourcemaps/basic.less test/sourcemaps/basic.public'
                ].join(' && ')
            },
        },
        ...
        clean: {
            ...
            'sourcemap-test': ['test/sourcemaps/*.public', 'test/sourcemaps/*.map'],
            sauce_log: ['sc_*.log']
        }
    });
